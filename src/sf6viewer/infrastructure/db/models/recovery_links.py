...
class RecoveryLinkModel(Base):
    ...
    job_id: Mapped[str] = mapped_column(ForeignKey("jobs.id", name="fk_recovery_links_job_id"), primary_key=True)
    ingestion_id: Mapped[str] = mapped_column(ForeignKey("ingestion_runs.id", name="fk_recovery_links_ingestion_id"), nullable=False)
