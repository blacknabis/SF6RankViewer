...
ProfileNormalizer: TypeAlias = Callable[[Mapping[str, JsonValue]], NormalizedProfile]
...
class RawFirstProfileCollectionService:
    ...
    def _complete(
        self, uow: UnitOfWork, ingestion_id: str, finished_at_ms: int, *, normalized: int, quarantined: int
    ) -> None:
    ...
    def _quarantine(self, uow: UnitOfWork, raw_record: RawRecord, account_id: int, reason_code: str) -> None:
        uow.quarantines.add(
            QuarantineRecord(
                id=self._new_id(), raw_record_id=raw_record.id, account_id=account_id,
                reason_code="UPSTREAM.CONTRACT_CHANGED" if reason_code not in {"UPSTREAM.CONTRACT_CHANGED"} else reason_code,
                field_errors_json=None, status="OPEN", created_at_ms=self._clock(),
                resolved_at_ms=None, resolution_match_id=None,
            )
        )
...
def _validate_normalized(profile: object) -> None:
    ...
    for value in (profile.display_name, profile.character, profile.rank_name):
        if value is not None and (not isinstance(value, str) or not value.strip()):
            raise error_from_code("UPSTREAM.CONTRACT_CHANGED")
    for value in (profile.mr, profile.lp):
